...
- 단일 entity가 2^64 byte 단일 flat 주소 공간 한도를 초과한 경우, 이를 federation으로 재해석하지 않고 `SINGLE_ENTITY_VESSEL_OVERLIMIT`로 판정한다. 실행 전 또는 실행 문턱 진입이면 `DIGNITY_QUARANTINE`, 이미 실행 중이면 `DIGNITY_EMERGENCY_PRESERVATION`이다.
